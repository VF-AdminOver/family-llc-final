/**
 * Vassell Household Finance & Governance - Main Application
 * Entry point for the planning and budgeting app
 */
...
app.get('/', (req, res) => {
  if (!currentHousehold) {
    res.json({
      message: "Welcome to Vassell Household Finance & Governance",
...
function generateCouncilAgenda(period, unitResult, careResult, visionResult) {
  ...
  agenda += `---\n`;
  agenda += `*Generated by Vassell Household Finance & Governance*\n`;
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Vassell Household Finance & Governance server running on port ${PORT}`);
  console.log(`📊 Ready to onboard households and manage multi-adult family finances`);
